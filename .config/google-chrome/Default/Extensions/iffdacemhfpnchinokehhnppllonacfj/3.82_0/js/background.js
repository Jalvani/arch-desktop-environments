...
chrome.runtime.onMessage.addListener(
    ...
    //WE DO NOT WANT TO REMEMBER THE TAB.ID OF HELP.HTML BECAUSE IT IS NOT A GMAIL TAB AND IT CLOSES ITSELF SO WE DO NOT HAVE A DESTINATION FOR RESPONSES
        if(sender.tab.url != chrome.extension.getURL('help.html')+"#signed_in"){
...
*/ else if (request.getUserSettings) {
    ...
    */
    ...
    /*          console.log(request.KMEventInfo); */
        ...
        } else if (request.checkIfPhoneNumCleanExists){
            sendResponse({
                confirmation: searchForCleanPhoneNumContact(request.phoneNumClean, request.phoneNum)
            })
        }
    }
);
...
function channel_api_token_setup() {
    console.log("about to request CAPI token");
        ...
        success: function(msg) {
            ...
            if (msg.indexOf("valid_token:") < 0) {
                ...
                socket.onopen = function() {
                    console.log("CAPI socket opened");
...
//                        console.log(message.data);
                        ...
                        $(tabsRunningContentScriptByID).each(function(){
                            ...
                            if (dataObject.new_content !== undefined) {
                                ...
                                console.log(dataObject.ack_processed);
                                chrome.tabs.sendMessage(CAPINotifDestination, {
                	    	...
                	    	//START NOTIFICATION OF GROUP MMS INCOMING
...
function loginBackInToMightyText(){
    chrome.tabs.create({
        url: signInUrlCRX,
        active: false
    });
...
function confirmTheseTabsRunContentScript(tabIDsToQuery){
    $(tabIDsToQuery).each(function(){
        var tabIDUnderScrutiny = this;
        var indexOfTabIDUnderScrutiny = tabsRunningContentScriptByID.indexOf(Number(tabIDUnderScrutiny));
//        console.log(Number(this));
    	chrome.tabs.sendMessage(Number(this), {
