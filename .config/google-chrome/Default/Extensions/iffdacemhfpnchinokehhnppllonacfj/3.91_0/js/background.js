...
function notifyUserOfNewFeaturesInThisUpdate(userEmail){
    ...
    chrome.storage.local.get(["initialFlag"], function(data) { 
        ...
        if(data["initialFlag"] === false){
            ...
            updateNotifContent[3.88] = "<b>What&#39;s new in GText (v3.88):</b><br></br><b>Emojis:</b> You can now send Emoticons and Emojis from GText! Try it out by clicking the smiley face icon and let us know what you think.<br></br><b>New Message Alert in Tab:</b> If you get an incoming SMS or MMS message and you&#39;re not in the Gmail tab, the tab title will flash to alert you.<br></br><b>Group Messaging:</b> You can now send & reply to group text message threads. More details <a target='blank' href='http://blog.mightytext.net/group-messaging-in-mightytext/'>here.</a>";
            updateNotifContent[3.89] = "<b>What&#39;s new in GText (v3.89):</b><br></br><b>Emojis:</b> You can now send Emoticons and Emojis from GText! Try it out by clicking the smiley face icon and let us know what you think.<br></br><b>New Message Alert in Tab:</b> If you get an incoming SMS or MMS message and you&#39;re not in the Gmail tab, the tab title will flash to alert you.<br></br><b>Group Messaging:</b> You can now send & reply to group text message threads. More details <a target='blank' href='http://blog.mightytext.net/group-messaging-in-mightytext/'>here.</a>";
            updateNotifContent[3.91] = "<b>What&#39;s new in GText (v3.91):</b><br></br><b>Emojis:</b> You can now send Emoticons and Emojis from GText! Try it out by clicking the smiley face icon and let us know what you think.<br></br><b>New Message Alert in Tab:</b> If you get an incoming SMS or MMS message and you&#39;re not in the Gmail tab, the tab title will flash to alert you.<br></br><b>Group Messaging:</b> You can now send & reply to group text message threads. More details <a target='blank' href='http://blog.mightytext.net/group-messaging-in-mightytext/'>here.</a>";
            ...
            chrome.storage.local.get(valToRetrieveFromChromeStorage, function(data) { 
                ...
                if(!data[valToRetrieveFromChromeStorage]){
                	...
                	chrome.tabs.sendMessage(currentGTextTabID, {
                        notifyUserOfUpdate: true,
                        updateContent: updateNotifContent[manifest.version]
                    }, function(response) {
                        console.log(response.confirmation);
                        if(response.confirmation == "notified user of update."){
                            ...
                            _gaq.push(["_trackEvent","CRX-Gmail","show_news_v." + manifest.version, userEmail]);
                    ...
                    console.log("the user has already been notified of the update.");
                }
...
chrome.tabs.onActivated.addListener(function(tabInfo){
    ...
    checkIfUserIsActiveInGtextTab("onActivatedListener");
})
...
*/
            console.error("ERROR: CALLING THIS SERVLET: "+ whichServletFunction);
            ...
            if (service == 'getUserInfo') {
                if ((reply_server.user) && (reply_server.user == "user not logged in")) {
                    ...
                    chrome.tabs.sendMessage(currentGTextTabID, {
                        ...
                        //NOT UPDATING USER OF UPDATE IN V3.91 BECAUSE IT IS FOR UPDATING GMAIL TRADEMARK INFRINGEMENTS
                        //notifyUserOfNewFeaturesInThisUpdate(userEmail);   
                        
/*
                        Category = "CRX_Gmail"
                        Action = "show_news_v3.89"
                        Label  = "maneesh@gmail.com"                   
*/
...
//            	alert("phoneNumClean: "+ phone_num_clean);
	...
	//        console.log("genericGetContactNameFromCaches is returning as the best contact name/number: " + num_or_name);
            	...
            	//we already createHTMLEquiv when we create the mycontacts array
...
//                        console.log(message.data);
                        ...
                        if (dataObject.new_content !== undefined) {
                            ...
                            if (dataType == 10) {
                                ...
                                $(tabsRunningContentScriptByID).each(function(){
                                ...
                                if(inboxOutbox === 60){
                                    ...
                                    checkIfUserIsActiveInGtextTab("incomingCAPI", "New Message");
                                }
                                ...
                                if (dataType != 83) {
                                    ...
                                    checkIfUserIsActiveInGtextTab("incomingCAPI", "Incoming Call");
    	                            $(tabsRunningContentScriptByID).each(function(){
                                        var CAPINotifDestination = Number(this);
                                        chrome.tabs.sendMessage(CAPINotifDestination, {
                                    ...
                                    if(dataType == 81){
                                        ...
                                        checkIfUserIsActiveInGtextTab("incomingCAPI", "Missed Call");
        	                            $(tabsRunningContentScriptByID).each(function(){
                                            var CAPINotifDestination = Number(this);
                                            chrome.tabs.sendMessage(CAPINotifDestination, {
                                ...
                                if(inboxOutbox == 60){
                                    checkIfUserIsActiveInGtextTab("incomingCAPI", "New Group Message");
                                }
                            } else if (dataType == 21){
                                console.log("incoming GroupMMS");
                                var contactHeaderNames = genericGetContactNameFromCaches(cleanPhoneNum, phoneNum);
...
function checkIfUserIsActiveInGtextTab(origin, messageToDisplay){
    chrome.tabs.query({
        active: true,
        currentWindow: true
    }, function(tabs) {
...
//        console.log(tabsRunningContentScriptByID);
        ...
        if(((gTextTabCheck > -1) && (origin == "onActivatedListener") && (contentScriptsAreTogglingTitles)) || (messageToDisplay == "Missed Call")) {//the user is already "active" within a tab running gtext or the user got a missed call, we also want to dismiss the interval
            //console.error("clear interval");
            contentScriptsAreTogglingTitles = false;
            togglePageTitle(false);
        } else if((gTextTabCheck == -1) && (origin == "incomingCAPI") && (!contentScriptsAreTogglingTitles)){//the user is running gtext, but not currently active in a tab that is running it.
            //console.error("start interval");
            contentScriptsAreTogglingTitles = true;
            togglePageTitle(true, messageToDisplay);
        }  
    });   
};
...
function togglePageTitle(initiateToggle, messageToDisplay){
    ...
    $(tabsRunningContentScriptByID).each(function(){
        ...
        var thisTabIDInt = Number(this);
        if((initiateToggle) && (contentScriptsAreTogglingTitles)){
            console.error("started a toggle on tabID: "+ thisTabIDInt);
            console.log(thisTabIDInt);
...
function openWebApp(phoneNumClean) {
    ...
    if(phoneNumClean){
        DestinationURL = DestinationURL + "/?1/#mode=quick&msgid=" + phoneNumClean;
    }
...
function confirmTheseTabsRunContentScript(tabIDsToQuery){
    $(tabIDsToQuery).each(function(){
        var tabIDUnderScrutiny = Number(this);
        var indexOfTabIDUnderScrutiny = tabsRunningContentScriptByID.indexOf(tabIDUnderScrutiny);
    	...
    	chrome.tabs.sendMessage(tabIDUnderScrutiny, {
            contentScriptCheck: true
                ...
                if (lastError && (indexOfTabIDUnderScrutiny > -1)) {
                    console.log("REMOVE THIS ID: "+ tabIDUnderScrutiny);
                    ...
                    console.log("Tab(s): " + tabIDsToQuery + " is not running the content script");
                }
