...
var themes = {
    'black': '<div style="' +
              ...
              '"><div class="lang" style="float:right;font-size:80%;"></div><div class="translation"></div><div class="additional"></div></div>',
    'white': '<div style="' +
                'max-width: 300px !important;' +
                'color: #111 !important;' +
                'opacity: 1 !important;' +
...
function _translate(text, from, to, options, callback) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
            ...
            //var r; eval('r = ' + xhr.responseText)
            var r = JSON.parse(xhr.responseText.replace().replace(/,,/gi, ',null,').replace(/,,/gi, ',null,'));
            callback(r, text);
        }
    }
    xhr.send();
...
function translate(text, port, options, context) {
    ...
    function translate(text, from) {
        ...
        _translate(text, from, to, options, function(result, text) {
            ...
            if(result[2] && result[2] == to && to != from) {
                return translate(text, to);
            }
            latest = result[2] ? result[2] : from;
            var text = [], i;
