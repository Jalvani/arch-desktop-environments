function PageInfo() {
  ...
  function afterInject() {
    ...
    /*if (window["isTourPage"]) {
      clearlyCallback(null, findArticleCallback);
      return;
    } */
    if (!hasCheckedArticle) {
      if (typeof initClearlyComponent__detect == "function" && typeof initClearlyComponent__next == "function") {
        window.ClearlyComponent__detect = {
          callbacks: {
            finished: function(data) {
              if (data && data._html) {
                cleanArticles.push(data._html);
              }
              window.ClearlyComponent__next.start();
              clearlyCallback(data, findArticleCallback);
            },
          },
          window: window,
          document: document,
        ...
        if (window.ClearlyComponent__detect) {
          window.ClearlyComponent__next = {
            callbacks: {
              newPageFound: function(page) {
                cleanArticles.push(page._html);
                articles.push(page._elements[0]);
                if (newPageCallback) {
                  newPageCallback(page._html, page._url, cleanArticles.length);
                }
              }
            },
            settings: {
              onCreateNextPagesContainerUseThisId: "evernoteClearlyNextContainer",
            },
          ...
          if (window.ClearlyComponent__next) {
            window.ClearlyComponent__next.createNextPagesContainer()
            window.ClearlyComponent__detect.start();
          } else {
            log.warn("failed to initiate clearly next component");
            clearlyCallback(null, findArticleCallback);
          }
        } else {
          log.warn("failed to initiate clearly detect component");
          clearlyCallback(null, findArticleCallback);
        }
      } else {
        log.warn("Couldn't find clearly!");
        clearlyCallback(null, findArticleCallback);
      }
    }
    ...
    // @TODO: Maybe clearly can get faster.
      ...
      log.warn("Page over 1mb, skipping article detection.");
      clearlyCallback(null, findArticleCallback);
    }
    else {
      findArticleCallback();
  ...
  function findArticle(callback) {
    ...
    if (!clearlyInjected) {
      if (searchEngine) {
        ...
        Browser.sendToExtension({ name: "injectScript", script: "clearly/js/detect.js" });
        ...
        //     chrome.extension.sendMessage({name: "injectScript", script: "clearly/js/detect.js"}, afterInject);
        //   }
        //   else {
        //     chrome.extension.sendRequest({name: "injectScript", script: "clearly/js/detect.js"}, afterInject);
        //   }
        // }
      }
    }
  ...
  function getDefaultArticle(callback, _newPageCallback) {
    newPageCallback = _newPageCallback;
    findArticle(function(){
      callback(article);
      if (newPageCallback) {
        newPageCallback(null, null, cleanArticles.length);
      }
    });
  ...
  function getAdditionalPages(callback) {
    return articles;
  }
  ...
  if (typeof Browser != "undefined") {
    Browser.addMessageHandlers({
      ...
      content_ready_6_0_2: readyRequestHandler,
  ...
  function getSelection() {
    ...
    for (var i = 0; i < docs.length; i++) {
      ...
      if (doc) {
        ...
        log.warn("iframe contained no Document object.");
      }
    }
  ...
  function getText(node, soFar, maxLen, keepText, topNode, removePunc) {
    if (node.nodeType == Node.TEXT_NODE) {
      if (keepText || !searchEngine) {
        ...
        var trimmed;
        if (removePunc) {
          trimmed = GlobalUtils.removePunctuation(text.trim()).replace(/\s+/g, " ");
        } else {
          trimmed = text.trim().replace(/\s+/g, " ");
        }
    ...
    if (node.nodeType == Node.ELEMENT_NODE) {
      if (banned.indexOf(node.nodeName.toLowerCase()) == -1) {
        // don't get the Clipper interface
        if (/^evernote.+Tools$/.test(node.id)) {
          return soFar;
        }
        for (var i = 0; i < node.childNodes.length; i++) {
          if (searchEngine) {
            ...
            if (matchesAllowedSelectorInSearchEngine(node) || keepText) {
              if (topNode) {
                soFar = getText(node.childNodes[i], soFar, maxLen, true, topNode, removePunc);
              }
              else {
                soFar = getText(node.childNodes[i], soFar, maxLen, true, node, removePunc);
              }
            }
            else {
              soFar = getText(node.childNodes[i], soFar, maxLen, false, null, removePunc);
            }
          }
          else {
            soFar = getText(node.childNodes[i], soFar, maxLen, false, null, removePunc);
  ...
  */
  ...
  function getRecommendationText(bookmark, callback) {
    var text = "";
    var MAX_LEN = 5000;
    var selection = getSelection();
    if (bookmark) {
      getDefaultArticle(function(art) {
        callback(getText(art, "", 510, false, null, false));
      });
    } else {
      if (selection) {
        ...
        div.appendChild(df);
        text = getText(div, "", MAX_LEN, false, null, true);
      }

      else if (article) {
        text = getText(article, "", MAX_LEN, false, null, true);
      }
      else {
        text = getText(document.body, "", MAX_LEN, false, null, true);
      }
      text = getTitle() + " " + text;
      return text;
    }
  ...
  function _getInfoRequestHandler(data, request, sender, sendResponse) {
    ...
    var response = {
      ...
      recommendationText: getRecommendationText(false),
  ...
  function setFavIconUrl(request, sender, sendResponse) {
  ...
  function msgHandlerInjectedScript(request, sender, sendResponse) {
    if (request.script == "clearly/js/detect.js") {
      Browser.sendToExtension({ name: "injectScript", script: "clearly/js/next.js" });
    } else if (request.script == "clearly/js/next.js") {
      afterInject();
    }
  }
  ...
  this.getAdditionalPages = getAdditionalPages;
  ...
  this.isGmailThread = isGmailThread;
  this.__defineGetter__("documentIsFrameset", function() {
    return documentIsFrameset;
  });
  this.__defineGetter__("favIconUrl", function() {
    return favIconUrl;
  });
