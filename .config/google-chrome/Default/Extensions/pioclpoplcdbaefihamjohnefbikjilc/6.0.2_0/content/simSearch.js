...
function SearchHelper() {
  ...
  function search(loggedIn) {
    ...
    if (loggedIn) {
      ...
      if (auth.businessId) {
        userType = "business";
      } else if (auth.premium) {
        userType = "premium";
      }
      url += "&userId=" + auth.userId + "&shardId=" + auth.shardId + "&client="
        + options.client + "&userType=" + userType;
    }
  ...
  function resizeIrameWidth() {
    ...
    if (iframe && rhs) {
      iframe.style.width = Math.min(Math.max(window.innerWidth - rhs.offsetLeft - 30, 280), 456) + "px";
    }
  }
