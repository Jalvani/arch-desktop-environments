...
try {
var script_injection_required = /chrome/i.test(SCRIPT_TYPE);;
    ...
    "177554951839":"Zynga Game Bar"
				...
				var weeks = days * 7;
					...
					try {
						...
						'			<form name="%key%" id="form_%key%" onSubmit="alert(\'1\');return false;">		\n'+
				...
				options.load(function(prefs){
					...
					if (options) {
						...
						a('disable_more_stories_handling','checkbox',false);
...
//						'#fbNotificationsList { max-height:'+(window.innerHeight-100)+'px !important; overflow:auto !important; } '+
					...
					// This is to fix the unsafeWindow bug "fix" in Chrome 27+
										var execute_in_page_scope = function(code,post_process_func) {
						...
						if (script_injection_required) {
							if (typeof code=="function") {
								if (dependencies) {
									code = "(function(){"+dependencies+";("+code.toString()+")();})();";
								}
								else {
									code = "("+code.toString()+")();";
								}
						...
						onIdLoad('pageNav',function(nav) {
							if ($('bfb_options_button')==null) {
								...
								if (li.style && li.style.setProperty) {
									...
									li.style.setProperty('float','right','important');
					...
					//					update_status('bfb_status_expanded',++count_expanded);
							...
							if (show_post_actions) {
								...
								bind(QS(d,'.bfb_post_action_save'),'click',function(e) {
									...
									QS(o,'.uiAttachmentTitle a, a.shareText',function(a) {
										url = a.href;
						...
						function fix_timestamps(o) {
							QSA(o,'abbr[data-utime][title]',function(a){ 
								...
								var inner_html = a.innerHTML;
								if (/\d+:\d+/.test(inner_html)) {
								...
								try {
									...
									if (container) {
										...
										if (options.get('show_nav_unblock_applications')) {
											append(container, el('li','sideNavItem stat_elem',null,null,createNavLink('Manage Blocking','/settings?tab=blocking',null,'navItem_apps','')) );
										}
									}
								}
								catch (e) { add_exception(e); }
...
//									}
						...
						if (fix_comments_on) {
							...
							(function() {
								...
								var default_text = " Use Tab &gt; Enter to Submit Comment";
								var ready_to_submit_text = "Press Enter To Submit Comment!";
								bind(document,'keydown',function(e) { 
									var t = target(e);
									if (t && t.tagName && t.tagName=="TEXTAREA" && t.name && t.name=="add_comment_text_text" && !parent(t,'#birthday_reminders_dialog')) {
										var button = null;
										...
										if (container) {
											...
											if (!button && fix_comments_on) {
												...
												var cb = el('input',null,{'type':'checkbox','title':'Click to stop using Tab-Enter to submit comments'},{'click':function(e){cancel_bubble(e);options.set('fix_comments',false);fix_comments_on=false;alert("Pressing Enter will now submit comments. You can re-enable the Tab-Enter functionality in Options.");removeChild(this.parentNode);if(t&&t.focus){t.focus();}}},null,{'margin':'0 5px 0 0'});
												cb.checked = true;
												insertFirst(button,cb);
										...
										if (button && fix_comments_on) {
											...
											else if (e.keyCode==13 && tab_pressed) {
												// Let Facebook process it to send the comment, but first let other code know a comment was submitted
												inform('sfx_comment_submitted');
											}
						...
						if (options.get('auto_switch_to_recent_stories')) {
							if (/sfx_switch=true/.test(window.location.href)) {
								...
								execute_in_page_scope(function() {
									if (unsafeWindow && unsafeWindow.requireLazy) {
										unsafeWindow.requireLazy(["UIIntentionalStream"],function(UIIntentionalStream) {
											var check = function() {
												if (!UIIntentionalStream.instance) { setTimeout(check,500); return false; }
												if (UIIntentionalStream.instance.currentFilterKey && UIIntentionalStream.instance.currentFilterKey!="h_chr") {
													if (document.querySelector('.uiStreamHeaderChronologicalForm')) {
...
//							});
						...
						if (options.get('notification_link_to_group_posts')) {
							bind(document.documentElement,'click',function(e) {
								var t = target(e);
								var a = parent(t,'.notifMainLink');
								if (a) {
									if (/\/groups\/[^\/]+\/\d+\//.test(a.href)) {
						...
						// Auto-clicking of "More Stories"
						if (!options.get('disable_more_stories_handling')) {
							execute_in_page_scope( {'fragment':fragment,'click':click,'bind':bind,'insertFirst':insertFirst,'insertAtPosition':insertAtPosition,'insertBefore':insertBefore,'insertAfter':insertAfter,'html':html,'append':append,'hasClass':hasClass,'countVisibleElements':countVisibleElements,'QSA':QSA,'is_visible':is_visible,'current_style':current_style}, function() {
								bind(unsafeWindow,'load',function() {
									var interval_id = null;
									var increment = +options.get('auto_click_more_times');
									var auto_click = options.get('auto_click_older_posts');
									var auto_click_at_bottom = options.get('auto_click_at_bottom2');
									var previousPostCount = 0;
									var loadMoreOnScrollAttempts = 0;
									var maxLoadMoreOnScrollAttempts = 5;
									if (!auto_click || !increment || increment<=0) { increment = 0; }

									if (unsafeWindow && unsafeWindow.requireLazy) {
										unsafeWindow.requireLazy(["UIIntentionalStream","OnVisible"],function(UIIntentionalStream,OnVisible) {
										
											var init = function() {
												var auto_load_count = 1;
												var max_auto_load_count = increment;
												var message_container = null;
												var ready_for_scroll = true;

												var inst = UIIntentionalStream.instance;
												if (!inst) { return; }
												inst.sfx_attached = true;

												if (auto_click_at_bottom) {
													inst.loadMoreOnScroll = function (qa, ra) {
														if (unsafeWindow.disableScrollLoad) return;
														var sa = function () {
															if (unsafeWindow.disableScrollLoad) return;
															var postCount=countVisibleElements('.bfb_processed');
															if (postCount==previousPostCount && loadMoreOnScrollAttempts++ > maxLoadMoreOnScrollAttempts) {
																var msg = fragment('Social Fixer has detected that Facebook is in an endless loop of loading more posts (which may be slowing down your browser), so auto-loading has been paused. To continue auto-loading more posts when you reach the bottom of the page, <a href="#" id="sfx_more_stories_on_scroll">click here</a>.',
																	{
																		'#sfx_more_stories_on_scroll': function(e) {
																			unsafeWindow.disableScrollLoad = false;
																			hide_message_container();
																			auto_load_count++;
																			loadMoreOnScrollAttempts = 0;
																			sa();
																		}
																	}
																);
																set_message(msg);
																unsafeWindow.disableScrollLoad = true;
															}
															else {
																previousPostCount = postCount;
																this.loadOlderPosts({delay_load_count: ra},true);
															}
														}.bind(this);
														var ua = unsafeWindow.document.getElementById('pagelet_stream_pager');
														if (ua) {
															this.currentScrollListener = new OnVisible(ua, sa, null, 200);
														} else sa();
													};
												}
												else {
												...
												var start_interval = function() { if (!interval_id) { interval_id = setInterval(auto_loader,1000); } };
												var stop_interval = function() { if(interval_id) { clearInterval(interval_id); interval_id=null; } };

												// Stop any previous interval checks
												stop_interval();
												
												// Monkey patch the existing loadOlderPosts() function with our own version
												inst._loadOlderPosts = inst.loadOlderPosts;
												inst.loadOlderPosts = function(qa,by_sfx) {
													var ra = inst.root.querySelector('div.fbStreamPager.hasMorePosts');
													if (!ra || hasClass(ra,'async_saving')) { return false; }
													if (by_sfx) {
														auto_load_count++;
													}
													if (qa) {
														qa.delay_load_count=12;
													}
													this._loadOlderPosts(qa);
												}

												var auto_loader = function() {
													if (!inst || !max_auto_load_count || max_auto_load_count<=0) { return; }
													if (auto_load_count > max_auto_load_count) {
														stop_interval();
														var msg = fragment('Social Fixer has auto-loaded '+(auto_load_count-1)+' pages of posts. <a href="#" id="sfx_more_posts_again">Load '+increment+' more pages</a>.',function(frag) {
															click(frag.querySelector('#sfx_more_posts_again'),function(e) {
																max_auto_load_count += increment;
																start_interval();
															},true,true);
														});
														set_message(msg);
														...
														var msg = fragment('Social Fixer is auto-loading more posts: '+auto_load_count+' of '+max_auto_load_count+'. <a href="#" id="sfx_more_posts_pause">Click to pause</a>',function(frag) {
															click(frag.querySelector('#sfx_more_posts_pause'),function(e) {pause();},true,true);
														});
														set_message(msg);
														inst.loadOlderPosts({},true);
													}
												};
												var pause = function() { 
													stop_interval();
													var msg = fragment('Social Fixer has paused auto-loading more posts ('+auto_load_count+' of '+max_auto_load_count+'). <a href="#" id="sfx_more_posts_resume">Click to resume</a>',function(frag) {
														click(frag.querySelector('#sfx_more_posts_resume'),function(e) {start_interval();},true,true);
													});
													set_message(msg);
												};
												var insert_message_container = function() {
													message_container = document.querySelector('#sfx_more_posts_message_container');
													if (message_container) { return; }
													var root = inst.root.querySelector("#pagelet_stream_pager").parentNode;
													if (root) {
														message_container = document.createElement('div');
														message_container.id = "sfx_more_posts_message_container";
														insertFirst(root,message_container);
													}
												};
												var show_message_container = function() {
													if (message_container) {message_container.style.display="block";}
												};
												var hide_message_container = function() {
													if (message_container) {message_container.style.display="none";}
												};
												var set_message = function(msg) {
													insert_message_container();
													show_message_container();
													if (!message_container) { return false; }
													if (typeof msg=="string") {
														html(message_container,msg);
													}
													else {
														html(message_container,"");
														append(message_container,msg);
													}
												};

												// Start up the auto-loading if the user has set the option
												if (increment>0) {
													start_interval();
												}
												else if (!auto_click_at_bottom){
													set_message("More Stories will not be loaded as you scroll down because this has been disabled in Social Fixer options.");
												}
											}

											init();
											// Check back repeatedly to make sure that the instance hasn't been re-instantiated
											setInterval(function() {
												if (UIIntentionalStream && UIIntentionalStream.instance && !UIIntentionalStream.instance.sfx_attached) {
													init();
												}
											},1000);
										});
						...
						onSelectorLoad('#group_pinned_412712822130938',function() {
