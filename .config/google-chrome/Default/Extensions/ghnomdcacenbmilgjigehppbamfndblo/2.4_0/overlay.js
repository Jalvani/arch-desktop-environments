...
var camelizer = {
  ...
  showLogin: function() {
    ...
    if (this.logged_in == true) {
      ...
      var anchor = "<a target='_blank' href='http://" + camelizer.context.domainStripped() + "/user/" + this.login + "'>"+this.login+"</a>";
      var text = i18n.t("t_signed_in_as", {"username": anchor});
      $("#loggedInAs").html(text);
      $("#loggedInAs").show();
    } else {
    ...
    onBeforeShow: function(e, pos) {
      ...
      $("#tooltip tbody td").each(function(index, td) {
        if (index != 0) {
          ...
          a = $("<a>", {"href": "#", "class": "discount "+discount_str, "rel": (suggest_amt/100).toFixed(2)}).text(price(suggest_amt)).click(function(e) {
...
Jed.prototype.t = function(key, el) { // el is either an html element or an options hash containing interpolations.
  try {
  ...
  if (str.indexOf("%{") > -1) {
    ...
    if (matches) {
      ...
      for(var i = 0; i<matches.length; i++) {
        var match = matches[i];
        key = match.substring(2, match.length-1);
