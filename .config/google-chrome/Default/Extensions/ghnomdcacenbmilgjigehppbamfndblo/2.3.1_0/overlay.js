...
var camelizer = {
  ...
  constructChartURL: function() {
    ...
    if (context.isBestbuy()) {
      ...
      if (file_name.length == 0) {
        throw new Error("no price types selected!");
      }
    ...
    this.ensureSelectedNotDisabled(function() {
      ...
      $('#camelizerChart').one('load', function() {
        ...
        if(camelizer.context.asin != camelizer.first_asin) {
          ...
          $('#productPageLink').find('a').attr('href', context.content_url);
          $("#footerRetailerName").text(context.retailer_name);
        }
  ...
  onLoad: function(url) {
    ...
    if (!camelizer.boundCommon) {
      bindCommon(function() {
        camelizer.processUrl(working_url);
      });
    } else {
      camelizer.processUrl(working_url);
    }
  ...
  processUrl: function(url) {
    if (url.substring(0,5) == "http:") {
      $("#camelizerListLink").show();
      getAsins(camelizer.context, function(hump) {
        var key = "camelizer_humps:"+url;
        tempCache.set(key, hump);
        camelizer.context = hump;
        if(hump.isWishlist()) {
  ...
  initiateProductLookup: function() {
    ...
    if (tempCache.get(key)) {
      ...
      this.getSecrets(function(secrets) {
        camelAsync.get(camelizer.context.request_url, secrets, function(data) {
          try {
  ...
  updateDisplay: function() {
    ...
    case R_PRODUCT_LOADED:
        camelizer.displayLoading("Product data downloaded, loading chart image...");
  ...
  showLogin: function() {
    ...
    if (this.logged_in == true) {
      ...
      $(".signupLogin").hide();
      $("#loggedInAs").show();
      $("#loggedInAs a").attr("href", "http://" + camelizer.context.domainStripped() + "/user/" + this.login).text(this.login);
    } else {
...
function bindCommon(bindCommonCallback) {
  ...
  $(".delete").click(function(e) {
    e.preventDefault();
    img = $(this);
    camelizer.confirm("Are you sure you want to delete this price watch?", function(result) {
      if (result == true) {
...
function localizedDollarsToCents(dollars) {
  locale = camelizer.context.locale.toLowerCase();
  if (locale == "es" || locale == "de" || locale == "fr") {
    decimal_sep = ",";
  } else {
    decimal_sep = ".";
  }
    ...
    onBeforeShow: function(e, pos) {
      ...
      $("#tooltip tbody td").each(function(index, td) {
        if (index != 0) {
          ...
          a = $("<a>").text(price(suggest_amt)).attr("href", "#").attr("class", "discount "+discount_str).attr("rel", (suggest_amt/100).toFixed(2)).click(function(e) {
