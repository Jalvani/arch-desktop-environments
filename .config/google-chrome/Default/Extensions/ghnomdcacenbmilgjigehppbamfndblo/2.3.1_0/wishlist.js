wishlistController = {
  ...
  getWishlists: function(callback) {
    camelAsync.get("http://www.amazon.com/registry/wishlist/", {}, function(response) {
      wishlists = [];
      if (response.status == 200) {
        str = response.response
        if (str.match(/<span.*?>Sign\sin<\/span>/)) { 
          // signed out
          url = str.replace(/(.|\n)*<a.*?href=["'](.*?\/ap\/signin.*?)['"].*?>(.|\n)*/, "$2");
          if (url != str) {
            return callback({"signin_link": url});
          }
        } else if (!str.match(/profileBox/) && str.match(/alt="Create\syour\sWish\sList"/)) { 
          // no wishlists
          return callback({"no_wishlist": true});
        } else if (!str.match(/profileBox/)) {
          // unknown error
  ...
  drawRow: function(wishlist) {
    ...
    var edit, del = $("<td>");
    row = $("<tr>");
    link = $("<td>").append($("<a>", {href: wishlist.url, target: '_blank', class: 'no_url_tracking amazon_wishlist_link'}).text(wishlist.name));
    items = $("<td>").addClass('wishlist_items').text(wishlist.items + " item"+ (wishlist.items != 1 ? "s" : ""));
    privacy = $("<td>");
    if (typeof this.imported_wishlists[wishlist.id] == "undefined") {
      if (wishlist.privacy == "private") {
        privacy = $("<span>").text("(" + wishlist.privacy + ") Your wishlist must be either shared or public to sync.");
      ...
      edit = $("<td>").append($("<a>", {href: 'http://' + camelizer.context.domainStripped() + '/wishlists/wishlist/'+imported_wishlist_id, target: '_blank', id: 'editWishlistLink', class: 'editWishlistLink'}).text("Edit"));
